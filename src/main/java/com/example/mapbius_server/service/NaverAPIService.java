...
public class NaverAPIService {
    ...
    public Object naverImageMethod(String query) throws URISyntaxException, UnsupportedEncodingException, JsonProcessingException {

        StringBuilder url = new StringBuilder("https://openapi.naver.com/v1/search/image");
        url.append("?").append("query").append("=").append(URLEncoder.encode(query, "UTF-8"));
        url.append("&").append("display").append("=").append("10");
        url.append("&").append("start").append("=").append("1");
        url.append("&").append("sort").append("=").append("date"); // 또는 sim(정확도)
        url.append("&").append("filter").append("=").append("small"); // 또는 sim(정확도)
